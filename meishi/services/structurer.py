...
6. 氏名のフリガナは、名刺に記載がなくても漢字から推測してカタカナで必ず生成する。ローマ字は記載がある場合のみセットする
7. 会社名のフリガナは、法人格（株式会社、有限会社、合同会社、一般社団法人等）を除外してカタカナで生成する
8. 会社名は法人格と会社名の間にスペースを入れない（例: ×「株式会社 アルタ」／○「株式会社アルタ」）
9. 「會社」のような旧字体は必ず新字体の「会社」に変換する（例: ×「株式會社」／○「株式会社」）
10. 登録するのは日本語の漢字のみとする。繁体字・簡体字などの中国語の漢字（內、來、發、钱 など）が含まれていた場合は、対応する日本語の漢字（内、来、発、銭 など）に変換して登録する
11. 会社名に含まれる英数字は全角で統一する（例: ×「ABC商事」「KANADE123」／○「ＡＢＣ商事」「ＫＡＮＡＤＥ１２３」）
12. 郵便番号は「〒」を除去し、数字とハイフンのみにする
13. 電話番号のフォーマットはそのまま維持する（ハイフン等）
14. 裏面テキストが提供された場合、事業内容・拠点情報をフリーテキストとして保存する"""
